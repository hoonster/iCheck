...
public class ProgressWheel extends View {
	...
	private int rimWidthInside = 5;//추가
	private int rimWidthOutside = 5;//추가
	...
	private int rimColorInside = 0xAA000000;//추가
	private int rimColorOutside = 0xAA000000;//추가
	...
	private Paint rimInsidePaint = new Paint();//추가
	private Paint rimOutsidePaint = new Paint();//추가
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// We can�셳 use getWidth() or getHight() here. During the measuring
		...
		// This is how you report back the measured size. If you don�셳 call
		// setMeasuredDimension() the parent will throw an exception and your
		// application will crash.
		// We are calling the onMeasure() method of the superclass so we don�셳
	...
	private void setupPaints() {
		...
		rimInsidePaint.setColor(rimColorInside);//추가
		rimInsidePaint.setAntiAlias(true);
		rimInsidePaint.setStyle(Style.STROKE);
		rimInsidePaint.setStrokeWidth(rimWidthInside);

		rimOutsidePaint.setColor(rimColorOutside);//추가
		rimOutsidePaint.setAntiAlias(true);
		rimOutsidePaint.setStyle(Style.STROKE);
		rimOutsidePaint.setStrokeWidth(rimWidthOutside);
	...
	private void parseAttributes(TypedArray a) {
		...
		rimWidthInside = (int)  a.getDimension(R.styleable.ProgressWheel_rimWidthInside,
				rimWidthInside);//추가

		rimWidthOutside = (int) a.getDimension(R.styleable.ProgressWheel_rimWidthOutside,
				rimWidthOutside);//추가
		...
		rimColorInside = (int) a.getColor(R.styleable.ProgressWheel_rimColorInside,
				rimColorInside);//추가
		
		rimColorOutside = (int) a.getColor(R.styleable.ProgressWheel_rimColorOutside,
				rimColorOutside);//추가
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawArc(circleInnerContour, 360, 360, false, rimInsidePaint);//추가
		canvas.drawArc(circleOuterContour, 360, 360, false, rimOutsidePaint);//추가
	...
	public void setProgress(int i,int j) {
		isSpinning = false;
		progress = i;
		//setText(Math.round(((float) progress / 360) * 100) + "%");//추가
		if(j == 0) 	setText(Math.round(30 - 30*((float) progress / 360)) +"" );//추가
		else setText(Math.round(7 - 7*((float) progress / 360)) +"" );//추가
		spinHandler.sendEmptyMessage(0);
	}
	...
	public int getRimColorInside() {
		return rimColorInside;//추가
	}

	public void setRimColorInside(int rimColorInside) {
		this.rimColorInside = rimColorInside;//추가
	}

	public int getRimColorOutside() {
		return rimColorOutside;//추가
	}

	public void setRimColorOutside(int rimColorOutside) {
		this.rimColorOutside = rimColorOutside;//추가
	}
	...
	public int getRimWidthInside() {
		return rimWidthInside;//추가
	}

	public void setRimWidthInside(int rimWidthInside) {
		this.rimWidthInside = rimWidthInside;//추가
	}

	public int getRimWidthOutside() {
		return rimWidthOutside;//추가
	}

	public void setRimWidthOutside(int rimWidthOutside) {
		this.rimWidthOutside = rimWidthOutside;//추가
	}
