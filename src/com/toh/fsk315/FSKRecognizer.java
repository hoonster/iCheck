...
class FSKRecognizer {
	private final static int FSK_SMOOTH = 2;
	...
	private ArrayList<FSKModemListener315> _dataReceivers;

	public FSKRecognizer() {
		halfWaveHistory = new int[FSK_SMOOTH];
		_recvBuf = new ByteArrayOutputStream();
		_dataReceivers = new ArrayList<FSKModemListener315>();
	}
	...
	private void commitBytes() {
		synchronized (_recvBuf) {
			...
			synchronized (_dataReceivers) {
				for (FSKModemListener315 lsn : _dataReceivers) {
	...
	boolean addDataReceiver(FSKModemListener315 lsn) {
		synchronized (_dataReceivers) {
			return _dataReceivers.add(lsn);
		}
	}

	boolean removeDataReceiver(FSKModemListener315 lsn) {
		synchronized (_dataReceivers) {
