...
class FSKRecognizer {
	private static int FSK_SMOOTH = 2; // 2 , note1
	...
	private ArrayList<FSKModemListener1225> _dataReceivers;

	public FSKRecognizer() {
		halfWaveHistory = new int[FSK_SMOOTH];
		_recvBuf = new ByteArrayOutputStream();
		_dataReceivers = new ArrayList<FSKModemListener1225>();
	}
	...
	private void commitBytes() {
		synchronized (_recvBuf) {
			...
			synchronized (_dataReceivers) {
				for (FSKModemListener1225 lsn : _dataReceivers) {
	...
	boolean addDataReceiver(FSKModemListener1225 lsn) {
		synchronized (_dataReceivers) {
			return _dataReceivers.add(lsn);
		}
	}

	boolean removeDataReceiver(FSKModemListener1225 lsn) {
		synchronized (_dataReceivers) {
